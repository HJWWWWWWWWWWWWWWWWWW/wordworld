...
NEW_MAPS = [
    # ── 沧澜帝国扩展 ──
    ("map_mo_city", "漠城", "沧澜帝国", "", 6, 5, "yes", "",
     "赤沙荒漠边缘的最后城池，冰皇海波东隐居于此。城内佣兵与探险者云集，沙漠情报与稀有材料时有流通。",
     "沙漠剧情关键节点"),
    ("map_stone_mo_city", "石漠城", "沧澜帝国", "", 7, 5, "yes", "",
     "深入沙漠前的补给重镇，蛇人族活动频繁。林烬在此遇见青鳞，命运的齿轮开始转动。",
     "青鳞相遇剧情"),
    ("map_black_rock_city", "黑岩城", "沧澜帝国", "", 4, 5, "yes", "",
     "沧澜帝国东部大城，设有炼药师公会分会。城内帮派林立，地下黑市暗中流通禁售药材。",
     "炼药师考核剧情"),
    ("map_salt_city", "盐城", "沧澜帝国", "", 5, 5, "yes", "",
     "以盐矿闻名的贸易城市，通往魔兽山脉东麓的门户。盐帮掌控城内大部分生意。",
     "沧澜帝国城市"),
    ("map_ghost_pass", "镇鬼关", "沧澜帝国", "", 6, 6, "no", "",
     "拱卫沧澜帝国北境的雄关，驻军日夜警戒着边境魔兽与敌国斥候。",
     "边境关隘"),
    ...
    # ── 暗角域扩展 ──
    ("map_black_seal_city", "黑印城", "暗角域", "", 20, 6, "yes", "",
     "暗角域核心大城，八扇门掌控的地下拍卖会名震暗角域。珍品功法、药材与禁物在此暗流涌动。",
     "暗角域拍卖城"),
    ("map_demon_flame_valley", "魔焰谷", "暗角域", "", 36, 10, "no", "",
     "暗角域三大势力之一，方言三老坐镇的魔道重地。谷中毒火终年不熄，与冷煜联手对抗迦南学院。",
     "暗角域清算战"),
    ("map_tianya_city", "天涯城", "暗角域边境", "", 26, 6, "yes", "",
    ...
    ("map_heaven_mountain_blood_pool", "天山血潭", "中州", "", 48, 10, "no", "",
     "天目山脉之巅的天然血池，地脉精华汇聚而成。能够淬炼筋骨、加速灵力凝练，中州青年才俊趋之若鹜。",
     "血潭核心"),
    ("map_sky_demon_sect", "天冥宗", "中州", "", 42, 8, "no", "",
     "中州老牌宗门，冥系功法诡异莫测。山门常年笼罩在幽暗雾气中，外人难窥其真面目。",
     "中州宗门"),
    ("map_death_corpse_mountains", "葬尸山脉", "中州", "", 55, 10, "no", "",
     "黑渊殿在中州的巢穴之一，尸气弥漫的死寂山岭。据说山脉深处隐藏着黑渊殿培养护法的秘地。",
     "黑渊殿据点"),

    # ── 黑渊殿扩展 ──
    ("map_heavenly_gang_hall", "天罡殿", "黑渊殿", "", 62, 10, "no", "",
     "黑渊殿总部核心大殿，殿主魂灭生掌控的灵魂中枢。殿内囚禁着无数被收集的强大灵魂体。",
     "黑渊殿总部"),

    # ── 妖火相关 ──
    ("map_demon_flame_plain", "妖火平原", "中州", "", 64, 8, "no", "",
     "净世白莲火空间现世前妖火气息外泄形成的焦土平原。奇异的火属性能量扭曲了整片区域。",
     "妖火前哨"),

    # ── 古帝洞府扩展 ──
    ("map_strange_flame_square", "源火广场", "古帝洞府", "", 78, 10, "no", "",
     "源帝以源火本源塑造的传承广场，二十二种源火的虚影在此永恒燃烧。帝品雏丹的气息弥漫其中。",
     "古帝传承核心"),
]
...
NEW_ENCOUNTERS = [
    ...
    enc("enc_mo_3", "map_mo_city",
        "沙漠商队在漠城集市招募护卫，领队高喊需要熟悉沙漠地形的灵者。报酬丰厚，但路程凶险。",
    ...
    enc("enc_black_rock_3", "map_black_rock_city",
        "城东帮派火并，硝烟中夹杂着灵力爆裂声。一个受伤的帮派成员跌跌撞撞向你求救。",
    ...
    enc("enc_black_seal_2", "map_black_seal_city",
        "黑印城地下拍卖会人声鼎沸，一件件珍品被推上展台。压轴拍品是一卷地阶灵技残卷。",
    ...
    enc("enc_demon_valley_1", "map_demon_flame_valley",
        "魔焰谷入口毒火肆虐，方言三老布下的护谷大阵将闯入者困于火海。火焰中夹杂着凄厉的哀嚎。",
        "以源火开路", "exp:+150,flag:demon_flame_valley_defeated=1",
        "寻找阵法破绽", "soul:+5"),
    enc("enc_demon_valley_2", "map_demon_flame_valley",
        "谷中密室中存放着魔焰谷多年积累的功法卷轴与炼药秘方，冷煜在此留下了海心焰的使用笔记。",
    ...
    enc("enc_tianya_2", "map_tianya_city",
        "罗家大小姐在街市上横冲直撞，一个卖玄炉老人人被她撞翻在地。围观者敢怒不敢言——罗家掌控着天涯城的虫洞。",
        "扶起老人", "reputation:+5,rel:npc_xuanlu_elder:+1",
    ...
    enc("enc_tianmu_1", "map_tianmu_mountains",
        "天目山脉能量潮汐如期而至，天地间的灵力如浪涛般冲刷着山脉。无数修炼者逆着能量流向上攀登。",
    ...
    enc("enc_flame_square_1", "map_strange_flame_square",
        "源火广场上二十二种源火虚影徐徐燃烧，每一种都散发着独特的能量波动。源帝的意志在其中低语。",
        "感悟古帝意志", "soul:+20,douqi:+30",
        "尝试收取源火虚影", "exp:+150"),
    enc("enc_flame_square_2", "map_strange_flame_square",
        "广场中央，帝品雏丹悬浮于光柱之中。灵帝级别的禁制层层叠叠地将其保护在核心。",
