...
REGION_ITEMS = [
    # 加玛帝国特产
    ("item_wutan_specialty", "乌坦城特产药材包", "consumable", "common", "false", "99", "50", "25",
     "", "hp:+20,stamina:+10",
     "乌坦城本地采集的混装药材，含凝血草、活气果等基础材料。仅在加玛帝国境内可获得。", "加玛帝国特产"),
    ("item_jia_ma_passport", "加玛通关文牒", "quest", "common", "false", "1", "100", "50",
     "", "reputation:+5",
     "加玛帝国官方颁发的通关文书，可免检通过帝国境内关隘。边境城市获取。", "加玛特产"),

    # 塔戈尔沙漠特产
    ("item_desert_compass", "沙漠星盘", "equipment", "uncommon", "false", "1", "300", "150",
     "", "soul:+3",
     "以沙晶打磨的导航星盘，能在沙暴中指引方向。塔戈尔沙漠独有。", "沙漠特产"),
    ...
    # 黑角域特产
    ("item_black_market_pass", "黑角域黑市令牌", "quest", "rare", "false", "1", "500", "250",
     "", "reputation:+3",
     "黑印城八扇门发放的黑市通行令牌，持有者可进入黑角域各大地下拍卖会。黑角域独有。", "黑角域特产"),
    ("item_forbidden_pill_fragment", "禁药配方残页", "material", "rare", "false", "1", "1000", "500",
     "", "alchemy:+5",
     "黑角域黑市中偶尔出现的禁药配方残页，记录着以寿命换取实力的禁忌炼药术。黑角域独有。", "黑角域特产"),
    ...
    ("item_alchemy_handbook", "炼药手札", "book", "uncommon", "false", "1", "500", "250",
     "", "alchemy:+5",
     "丹塔发行的炼药师入门手札，记录着基础丹方与控火技巧。丹域各城有售。", "丹域特产"),

    # 兽域特产
    ("item_beast_blood_essence", "魔兽精血瓶", "material", "rare", "true", "5", "500", "250",
     "", "douqi:+10",
     "高阶魔兽的精炼血液，蕴含浓郁斗气。可用于炼药或直接吸收。兽域独有。", "兽域特产"),
    ...
    ("item_dragon_scale", "龙鳞碎片", "material", "legendary", "false", "1", "3000", "1500",
     "", "def:+20,douqi:+30",
     "太虚古龙蜕落的鳞片碎片，蕴含龙族威压。可用于炼制顶级护甲。龙岛独有。", "龙岛特产"),
    ...
    ("item_demon_flame_crystal", "妖火结晶", "material", "legendary", "false", "1", "4000", "2000",
     "", "douqi:+40",
     "净莲妖火焚烧空间形成的火晶，蕴含精纯的异火能量。妖火空间独有。", "妖火空间特产"),
...
NEW_COMMERCE_MAPS = [
    ...
    ("map_ye_market", "叶城坊市", "叶城", "", 40, 4, "yes", "",
     "叶城中心交易区，叶家丹药铺占据了最好的位置。丹会期间，这里变成了炼药师的小型交流会场。", "商业"),
    ...
    ("map_daling_inn", "大岭城客栈", "大岭城", "", 8, 3, "yes", "",
     "赶往黑角域的旅人在此歇脚，客栈里的每一张桌子都在讨论黑角域的凶险与机遇。", "商业"),

    # ── 宗门交易区 ──
    ("map_burning_valley_market", "焚炎谷坊市", "焚炎谷", "", 40, 5, "yes", "",
     "焚炎谷外围的交易区，谷中炼器师出售火属性武器与护甲。偶尔能买到唐震亲手淬炼的火纹钢。", "商业"),
    ("map_star_pavilion_market", "星陨阁坊市", "星陨阁", "", 40, 4, "yes", "",
     "星陨阁为弟子与访客设立的交易区，丹药品质上乘——毕竟是药老的门派。", "商业"),
...
NEW_ENCOUNTERS = [
    ...
    enc("enc_inner_market_1", "map_inner_market",
        "一个强榜前十的学长在摊位上出售他淘汰的玄阶斗技卷轴。'修炼到圆满就没什么用了，便宜卖了。'",
        "买下斗技", "silver:-30,exp:+50", "讨价还价", "silver:-20,exp:+40"),
    ...
    enc("enc_ye_market_1", "map_ye_market",
        "丹会期间，坊市变成了小型交流会。几个炼药师正围着一尊药鼎争论丹方中火候的控制技巧。",
        "加入讨论", "alchemy:+3,soul:+2", "旁听学习", "alchemy:+1"),
    # ── 天黄城坊市 ──
    enc("enc_th_market_1", "map_tianhuang_market",
        "天黄城坊市的一个古族摊位吸引了你的注意——摊主在出售远古遗迹中出土的功法残片，价格令人咋舌。",
    ...
    enc("enc_dl_market_1", "map_daling_market",
        "边境集市上，一个从黑角域逃回来的伤者正在出售他的'生存经验'——一卷手绘的黑角域势力分布图。",
        "买下地图", "silver:-15,soul:+3", "向伤者打听详情", "soul:+2"),
    enc("enc_dl_inn_1", "map_daling_inn",
        "客栈中一个刚离开黑角域的佣兵正在讲述他的遭遇：'那鬼地方根本没有规则！实力就是一切！'",
    ...
    enc("enc_sp_market_1", "map_star_pavilion_market",
        "药老亲传的几名弟子在坊市中出售高阶丹药——品质远超市面流通品，价格也相当公道。",
...
NEW_GATHERING = [
    # ── 加玛帝国特产采集 ──
    gather("gather_wutan_specialty", "map_wutan_commercial_street", "item_wutan_specialty", 35, 1, 3, "", "", "", "加玛帝国", "乌坦城特产"),
    gather("gather_rock_specialty", "map_black_rock_market", "item_wutan_specialty", 30, 1, 2, "", "", "", "加玛帝国", "黑岩城特产"),
    gather("gather_passport_border", "map_jia_ma_border", "item_jia_ma_passport", 15, 1, 1, "", "", "", "加玛帝国", "通关文牒"),
    gather("gather_passport_ghost", "map_ghost_pass", "item_jia_ma_passport", 20, 1, 1, "", "", "", "加玛帝国", "通关文牒"),

    # ── 沙漠特产采集 ──
    gather("gather_desert_compass", "map_desert_camp", "item_desert_compass", 15, 1, 1, "", "", "", "塔戈尔沙漠", "沙漠星盘"),
    ...
    # ── 黑角域特产采集 ──
    gather("gather_black_market_pass", "map_black_seal_auction", "item_black_market_pass", 10, 1, 1, "", "", "", "黑角域", "黑市令牌"),
    gather("gather_forbidden_pill", "map_black_herb_market", "item_forbidden_pill_fragment", 5, 1, 1, "", "", "", "黑角域", "禁药残页"),
