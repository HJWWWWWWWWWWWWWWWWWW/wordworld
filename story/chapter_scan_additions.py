...
# ══════════════════════════════════════════════════════════
NEW_NPCS = [
    ("npc_liu_ling", "柳翎", "加玛帝国炼药师公会，法犸弟子，丹王传人"),
    ("npc_gu_ni", "谷尼", "乌坦城二品炼药师，米特尔拍卖场鉴定师"),
    ...
    ("npc_gu_qian", "古谦", "古族长老"),
    ("npc_gu_xu", "古虚", "古族强者"),
    ("npc_gu_dao", "古道", "古族三仙之一"),
    ("npc_yan_huo", "炎火", "炎族强者"),
    ("npc_yan_yao", "炎耀", "炎族"),
    ("npc_lei_yun", "雷云", "雷族强者"),
    ("npc_lei_dian", "雷电", "雷族强者"),
    ("npc_jiutian_zun", "九天尊", "魂殿九大天尊统称"),
]
...
NEW_MAPS = [
    # ── 加玛帝国→黑角域途中 ──
    ("map_daling_city", "大岭城", "加玛帝国", "", 8, 5, "yes", "",
     "拱卫加玛帝国东境的边陲重镇，是离开加玛前往黑角域的必经之路。往来商旅在此补给，守军戒备森严。",
     "边境枢纽"),
    ("map_black_domain_plain", "黑域大平原", "黑角域", "", 18, 12, "no", "",
     "连接加玛帝国方向与黑角域核心地带的辽阔平原。法则混乱，亡命徒横行，没有任何规则可言。",
     "黑角域入口"),

    # ── 黑角域组织 ──
    ("map_blood_sect", "血宗", "黑角域", "", 24, 8, "no", "",
     "黑角域老牌势力，血宗秘法以血炼之术著称。范痨坐镇的阴森山门，血池翻涌令人作呕。",
     "黑角域势力"),
    ("map_eight_gates", "八扇门", "黑印城", "", 20, 5, "yes", "",
     "黑印城地下秩序的掌控者，八扇铜门背后是黑角域最大的地下拍卖网络。",
     "黑印城势力"),
    ("map_pan_gate", "磐门", "迦南内院", "", 22, 4, "yes", "",
     "萧炎在内院创建的新生势力，以团结新生学员为核心。磐门之名寓意坚如磐石、不可动摇。",
     "内院组织"),
    ("map_xiao_gate", "萧门", "迦南内院", "", 25, 4, "yes", "",
     "萧炎离开后磐门发展壮大的延续，成为内院最强新生势力之一。萧门弟子遍布内院。",
     "内院组织进阶"),
    ...
    ("map_wan_yao_mountains", "万药山脉", "丹域", "", 54, 8, "no", "",
     "丹界附近的原始山脉，盛产万年灵药。丹塔在此设有药兽保护区，进入需丹塔许可。",
     "丹域药山"),
    ...
    # ── 丹塔 ──
    ("map_small_dan_tower", "小丹塔", "丹塔", "", 65, 6, "yes", "",
     "丹塔内部最隐秘的圣地，只有丹塔最高层知晓其存在。大长老在此闭关，守护炼药师一脉的至高传承。",
     "丹塔核心圣地"),
    ("map_black_emperor_pavilion", "黑皇阁", "黑皇城", "", 28, 5, "yes", "",
     "黑皇宗接待各方强者的奢华会馆，大厅宽广如广场。黑角域最顶级的交易在此暗中进行。",
     "黑皇宗会馆"),

    # ── 葬天山脉 ──
    ("map_burial_sky_mountains", "葬天山脉", "中州", "", 88, 15, "no", "",
     "远古大战形成的破碎山脉，无数强者葬身于此。魂天帝在此布置最终战场，天地为之变色。",
     "最终战场所在地"),
]
...
NEW_ENCOUNTERS = [
    ...
    enc("enc_daling_2", "map_daling_city",
        "城中商队正在招募护卫，即将穿越边境前往黑角域方向。领队强调路途凶险，亡命徒与魔兽并存。",
    ...
    enc("enc_pan_gate_1", "map_pan_gate",
        "磐门总部内新生们正在切磋斗技，墙上挂着磐门的规章——团结、互助、不欺压同门。",
    ...
    enc("enc_xiao_gate_1", "map_xiao_gate",
        "萧门大厅中悬挂着萧炎的画像，弟子们在此议事。一名刚从黑角域回来的弟子带来了韩枫的消息。",
    ...
    enc("enc_wan_yao_2", "map_wan_yao_mountains",
        "丹塔在此设有药兽园入口，一名丹塔执事正在审核入山采药者的资质。",
    ...
    enc("enc_demon_blood_1", "map_heaven_demon_blood_pool",
        ...
        "以异火护体进入血池", "douqi:+30,exp:+200", "在池边收集血晶", "item:+item_huangquan_blood_crystal"),
    ...
    enc("enc_small_dan_1", "map_small_dan_tower",
        "小丹塔中，大长老正在考核你的炼药造诣。一尊古朴的药鼎中，九色丹雷正在凝聚。",
        "完成炼制", "alchemy:+8,soul:+5", "请教大长老", "alchemy:+5,rel_player_yao_lao:+0"),
    enc("enc_small_dan_2", "map_small_dan_tower",
        "小丹塔的藏经阁中收藏着失传的远古丹方。守阁人递给你一卷《帝丹谱》的残卷。",
        "研读丹方", "alchemy:+10,soul:+8", "抄录带走", "alchemy:+6"),

    # ── 黑皇阁 ──
    enc("enc_black_pavilion_1", "map_black_emperor_pavilion",
        "黑皇阁大厅中灯火辉煌，黑角域各方势力首领围坐圆桌。莫天行正展示一件压轴的远古异宝。",
